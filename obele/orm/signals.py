...
class Signal:
	...
	Args:
	    name: Human-readable signal name (used in `repr`).
	    providing_args: Documentation-only list of keyword argument names
	        that `send` will provide to receivers.
	"""

	__slots__ = ("name", "providing_args", "_receivers", "_lock")

	def __init__(self, name: str = "", providing_args: list[str] | None = None) -> None:
		"""Create an empty signal with no connected receivers."""
		self.name = name
		self.providing_args = providing_args or []
		self._receivers: dict[Any, list[_Receiver]] = defaultdict(list)
		self._lock = threading.Lock()

	def connect(self, receiver: _Receiver, *, sender: type[Model] | None = None, weak: bool = False) -> None:
	...
	def disconnect(self, receiver: _Receiver, *, sender: type[Model] | None = None) -> bool:
	...
	def __repr__(self) -> str:
		"""Return a debug representation with the total receiver count."""
...
def receiver(signal: Signal | list[Signal], *, sender: type[Model] | None = None) -> Callable[[_Receiver], _Receiver]:
	...
	def decorator(fn: _Receiver) -> _Receiver:
		"""Connect `fn` to each target signal and return it unchanged."""
		for sig in signals:
...
__all__ = ["Signal", "pre_save", "post_save", "pre_delete", "post_delete", "pre_create", "post_create", "receiver"]
