...
from .exceptions import FieldValidationError, MigrationError, RecordNotFoundError
from .fields import _MISSING, Field, ForeignKeyField, IntegerField
from .query import QuerySet
from .signals import post_create, post_delete, post_save, pre_create, pre_delete, pre_save
...
class ReverseRelationManager:
	...
	def __init__(self, instance: Model, related_model: type[Model], field_name: str, accessor_name: str) -> None:
		"""Bind the manager to `instance` and the related model's FK field."""
	...
	def __repr__(self) -> str:
		"""Return a debug representation naming the owner, related model, and pk."""
		return (
	...
	def _prefetched(self) -> list[Model] | None:
		"""The prefetched rows cached on the owning instance, or `None`."""
		return self.instance.__dict__.get(f"_prefetch_{self.accessor_name}")

	def _queryset(self) -> QuerySet:
		"""Build a QuerySet of related rows filtered by the owner's primary key.

		Raises:
		    RecordNotFoundError: If the owning instance has not been saved.
		"""
		pk = self.instance.pk
		if pk is None:
	...
	def all(self) -> list[Model]:
		"""Return the related rows, serving prefetched data when available."""
	...
	def count(self) -> int:
		"""Return the number of related rows, counting prefetched data if cached."""
		cached = self._prefetched
	...
	def __iter__(self):
		"""Iterate the related rows, using prefetched data when available."""
		cached = self._prefetched
...
class ReverseRelationDescriptor:
	"""Installed on the target Model; returns a bound `ReverseRelationManager`."""
	...
	def __init__(self, related_model: type[Model], field_name: str, accessor_name: str) -> None:
		"""Store the related model and FK field the reverse accessor resolves to."""
		self.related_model = related_model
		self.field_name = field_name
		self.accessor_name = accessor_name

	def __get__(self, instance: Model | None, owner: type[Model]) -> Any:
		"""Return the descriptor on class access, else a bound `ReverseRelationManager`."""
		if instance is None:
			return self
	...
	for source_model in _model_registry.values():
		for field_name, field in source_model._fields.items():
			...
			if existing is not None and not isinstance(existing, ReverseRelationDescriptor):
				raise ValueError(f"Cannot register reverse relation '{accessor_name}' on {related_model.__name__}: attribute already exists")
...
class MetaModel(type):
	...
	def __new__(mcs, cls_name: str, bases: tuple[type, ...], namespace: dict[str, Any]) -> type:
		"""Build a model class: gather fields, resolve the PK, and precompute SQL plans."""
	...
	def __getattr__(cls, name: str) -> Any:
		"""Proxy QuerySet API names (`filter`, `all`, ...) to a fresh queryset."""
	...
	def _build_plans(cls: type) -> None:
		...
		cls._hydration = tuple(
			(name, f.column_name, f.to_python, f.cache_attr_name if isinstance(f, ForeignKeyField) else None) for name, f in fields.items()
		)
		...
		def insert_sql(names: Sequence[str]) -> str:
			"""Build an `INSERT` statement for the given field `names`."""
			cols = ", ".join(fields[n].column_name for n in names)
			marks = ", ".join("?" for _ in names)
			return f"INSERT INTO {cls.table_name} ({cols}) VALUES ({marks})"

		cls._insert_sql = insert_sql(cls._non_pk_field_names) if cls._non_pk_field_names else f"INSERT INTO {cls.table_name} DEFAULT VALUES"
...
class Model(metaclass=MetaModel):
	...
	def __init__(self, **kwargs: Any) -> None:
		"""Initialize field values from `kwargs`, applying defaults and snapshotting.

		Field descriptors validate and coerce assigned values.  The reserved
		keys `_persisted` and `_annotations` seed persistence state.
		"""
	...
	def _take_snapshot(self) -> None:
		"""Record current non-PK field values as the baseline for dirty tracking."""
	...
	def dirty_fields(self) -> dict[str, Any]:
		...
		return {name: value for name in self._non_pk_field_names if (value := self.__dict__.get(name)) != snapshot.get(name)}

	@property
	def is_dirty(self) -> bool:
		"""Whether any field has changed since the last load or save."""
		return bool(self.dirty_fields)

	def __repr__(self) -> str:
		"""Return a debug representation with the class name and primary key."""
		return f"<{type(self).__name__} pk={self.pk}>"

	def __eq__(self, other: object) -> bool:
		"""Compare by primary key; unsaved instances are equal only by identity."""
	...
	def __hash__(self) -> int:
		"""Hash by class name and primary key."""
		return hash((type(self).__name__, self.pk))

	@classmethod
	def _create_table_sql(cls, if_not_exists: bool = True) -> str:
		"""Build the `CREATE TABLE` statement including unique and check constraints."""
	...
	def _create_index_sqls(cls) -> list[str]:
		"""Build `CREATE INDEX` statements for indexed fields and index groups."""
		indexes = [
			f"CREATE INDEX IF NOT EXISTS idx_{cls.table_name}_{f.column_name} ON {cls.table_name} ({f.column_name})"
		...
		for group in cls._index_together:
			...
			if cols:
				indexes.append(f"CREATE INDEX IF NOT EXISTS idx_{cls.table_name}_{'_'.join(cols)} ON {cls.table_name} ({', '.join(cols)})")
		return indexes
	...
	def _table_exists(cls) -> bool:
		"""Return whether this model's table exists in the database."""
		return Database.fetchone("SELECT name FROM sqlite_master WHERE type='table' AND name=?", [cls.table_name]) is not None

	@classmethod
	def _migration_default_value(cls, field: Field) -> Any:
		"""Resolve the value used to backfill a newly added column during migration.

		Returns `_MISSING` when the database can supply the value itself.

		Raises:
		    MigrationError: For a new non-nullable column that has no default.
		"""
		...
		if field.nullable:
			return _MISSING
		raise MigrationError(f"Cannot migrate {cls.__name__}: new non-nullable column '{field.column_name}' has no default")
	...
	def migrate(cls, *, rename_fields: dict[str, str] | None = None, create_if_missing: bool = True) -> None:
		...
		with Database.transaction() as conn:
			...
			if insert_columns:
				conn.execute(
				f"INSERT INTO {cls.table_name} ({', '.join(insert_columns)}) SELECT {', '.join(select_expressions)} FROM {temp_table}",
					params,)
	...
	def _validate_all(self) -> None:
		"""Validate every field's current value, raising on the first failure."""
		for name, field in self._fields.items():
	...
	def _insert(self) -> None:
		"""Run the `INSERT` for a new row and capture the generated primary key."""
		d = self.__dict__
		fields = self._fields
	...
	def _update(self) -> None:
		"""Run the `UPDATE` for changed fields only; a no-op when nothing is dirty."""
		...
		Database.execute(f"UPDATE {self.table_name} SET {', '.join(set_parts)} WHERE {self._pk_col} = ?", values)
	...
	def delete(self) -> None:
		...
		Database.execute(f"DELETE FROM {self.table_name} WHERE {self._pk_col} = ?", [self.pk])
	...
	def refresh(self) -> None:
		...
		row = Database.fetchone(f"SELECT * FROM {self.table_name} WHERE {self._pk_col} = ?", [self.pk])
		if row is None:
	...
	def get_by_pk(cls, pk: Any) -> Model:
		...
		if cls._queryset.__func__ is Model._queryset.__func__:
			...
			row = Database.fetchone(f"SELECT * FROM {cls.table_name} WHERE {cls._pk_col} = ?", [pk])
			if row is None:
	...
	async def aget_by_pk(cls, pk: Any) -> Model:
		"""Async version of `get_by_pk`."""
		return await athread(cls.get_by_pk, pk)

	@classmethod
	def _build_upsert(
		cls, kwargs: dict[str, Any], conflict_fields: str | Sequence[str] | None, update_fields: Sequence[str] | None
	) -> tuple[str, list[Any], list[str], Model]:
		"""Build the `INSERT ... ON CONFLICT` SQL, values, conflict names, and instance."""
		instance = cls(**kwargs)
		...
		d = instance.__dict__
		insert_names = [n for n, f in cls._fields.items() if not (f.primary_key and d.get(n) is None and isinstance(f, IntegerField))]
		...
		if update_names:
			set_sql = ", ".join(f"{cls._fields[n].column_name} = excluded.{cls._fields[n].column_name}" for n in update_names)
	...
	@classmethod
	def upsert(
		cls, *, conflict_fields: str | Sequence[str] | None = None, update_fields: Sequence[str] | None = None, **kwargs: Any
	) -> Model:
	...
	def _bulk_rows(cls, items: list[dict[str, Any]], validate: bool) -> dict[tuple[str, ...], list[tuple[int, list[Any]]]]:
		...
		if errors:
			raise FieldValidationError(f"Validation failed for {len(errors)} field(s) in bulk_create:\n" + "\n".join(errors))
		return groups
	...
	def bulk_create(cls, items: list[dict[str, Any]], *, validate: bool = True) -> list[Model]:
		...
		with Database.transaction():
			for names, rows in groups.items():
				if not names:
					for idx, _ in rows:
						row = Database.execute(f"INSERT INTO {cls.table_name} DEFAULT VALUES RETURNING {cls._returning_cols}").fetchone()
