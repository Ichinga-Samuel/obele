...
from obele import (
    ...
    ForeignKeyField, JSONField, DateTimeField,
    Q, F, Value, Func, RawSQL, Subquery, Count, Sum, Avg, Min, Max,
    QuerySet,
    RecordNotFoundError, MultipleResultsError, FieldValidationError,
)
...
class TestQObjects:
    ...
    def test_q_nested(self):
        ...
        QAuthor.create(name="Bob", age=25)
        QAuthor.create(name="Carol", age=35)
        result = QAuthor.filter(
            (Q(name="Alice") | Q(name="Carol")) & Q(age__gte=30)
        ).all()
        assert len(result) == 2
...
class TestAnnotateGroupBy:
    def test_annotate_with_count(self):
        ...
        results = QAuthor.join("qbooks").annotate(
            book_count=Count(F("qbooks__id"))
        ).group_by("id").all()
    ...
    def test_annotate_with_sum(self):
        ...
        results = QAuthor.join("qbooks").annotate(
            total_pages=Sum(F("qbooks__pages"))
        ).group_by("id").all()
    ...
    def test_having(self):
        ...
        results = QBook.annotate(
            cnt=Count("id")
        ).group_by("author").having(cnt__gte=2).all()
...
class TestJoinsAndRelations:
    ...
    def test_explicit_join(self):
        a = QAuthor.create(name="Alice")
        QBook.create(title="B1", author=a)
        result = QBook.join("author").filter(
            author__name="Alice"
        ).all()
        assert len(result) == 1
...
# ---------- QuerySet Chaining / Immutability --------------------------------
