...
class TestCRUD:
	...
	def test_refresh(self):
		...
		Database.execute(f"UPDATE users SET name = 'Eva' WHERE id = ?", [u.id])
		u.refresh()
		assert u.name == "Eva"
...
class TestBulkOperations:
	...
	def test_bulk_update(self):
		users = User.bulk_create([{"name": "A", "age": 10}, {"name": "B", "age": 20}])
		for u in users:
			u.age = u.age + 100
	...
	def test_update_or_create(self):
		user, created = User.update_or_create(defaults={"age": 30}, name="Alice")
		assert created is True
		user2, created2 = User.update_or_create(defaults={"age": 40}, name="Alice")
		assert created2 is False
		assert user2.age == 40
