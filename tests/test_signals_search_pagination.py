...
class TestSearchRebuild:
	...
	def setup(self):
		...
		self.idx = SearchIndex(Article, fields=["title", "body"], content_sync=False)
		self.idx.create()
		yield
		self.idx.drop()
	...
	def test_rebuild_after_content_sync_insert(self):
		...
		idx2 = SearchIndex(Article, fields=["title", "body"], fts_table="ssp_articles_fts2", content_sync=True)
		idx2.create()
		try:
...
class TestSearchContentSyncTriggers:
	...
	def setup(self):
		Article.create_table()
		self.idx = SearchIndex(Article, fields=["title", "body"], content_sync=True)
		self.idx.create()
		yield
		self.idx.drop()
	...
	def test_triggers_exist_in_sqlite(self):
		...
		rows = Database.fetchall("SELECT name FROM sqlite_master WHERE type='trigger' AND name LIKE ?", [f"{self.idx.fts_table}_%"])
...
class TestSearchCustomFtsTable:
	...
	def test_custom_name(self):
		idx = SearchIndex(Article, fields=["title", "body"], fts_table="my_custom_fts_table")
		idx.create()
		try:
...
class TestSearchDrop:
	...
	def test_drop_removes_fts_table(self):
		...
		idx.create()
		# Verify table exists
		row = Database.fetchone("SELECT name FROM sqlite_master WHERE type='table' AND name=?", [idx.fts_table])
	...
	def test_drop_removes_triggers(self):
		...
		idx.create()
		idx.drop()
		rows = Database.fetchall("SELECT name FROM sqlite_master WHERE type='trigger' AND name LIKE ?", [f"{idx.fts_table}_%"])
		assert len(rows) == 0
...
class TestPaginationAllPagesCoverAllItems:
	...
	def test_all_items_covered(self):
		all_ids = set()
		per_page = 5
		total_pages = PagItem.order_by("rank").paginate(page=1, per_page=per_page).pages
...
class TestCursorPaginationForward:
	...
	def test_forward_traversal_covers_all(self):
		...
		while page.has_next:
			page = PagItem.order_by("id").cursor_paginate(per_page=10, after=page.end_cursor)
			all_ids.extend(item.id for item in page)
...
class TestCursorPaginationAfter:
	...
	def test_after_skips_items(self):
		...
		second = PagItem.order_by("id").cursor_paginate(per_page=5, after=first.end_cursor)
	...
	def test_after_with_no_more_items(self):
		...
		page = PagItem.order_by("id").cursor_paginate(per_page=5, after=last_id)
		assert page.items == []
		assert page.has_next is False
